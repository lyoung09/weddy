...
class BudgetHeaderWidget extends BaseStatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return widgetFactory.createHeaderCard(context,
        widget: GenericProvider.createWidget<int>(provider: budgetPriceProvider,
            (state) {
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          createPredictBudgetWidget(
                              totalAmountProvider, budgetPriceProvider),
                        ],
                      )
                    ],
                  ),
  ...
  Widget createPredictBudgetWidget(GenericProvider<int> totalAmountProvider,
      GenericProvider<int> budgetPriceProvider) {
    return GenericProvider.createWidget<int>(provider: totalAmountProvider,
        (state) {
      return Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        AutoSizeText(
            '${getBudgetDiffValue(state.value, budgetPriceProvider.value)}%',
            style: TextStyle(
                color: state.value! > budgetPriceProvider.value!
                    ? Colors.redAccent
                    : const Color(0xFF005666))),
  ...
  Widget createProgressbarWidget(GenericProvider<double> progressProvider) {
    return Column(
      ...
      children: [
        ...
        GenericProvider.createWidget<double>(provider: progressProvider,
            (state) {
  ...
  Widget createSliderWidget(
      BuildContext context, GenericProvider<double> progressProvider) {
    return GenericProvider.createWidget<double>(provider: progressProvider,
        (state) {
      return Slider(
        min: 0.0,
        max: 100.0,
