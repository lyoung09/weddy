...
class ScrapBottomModalFirst extends StatefulWidget {
  int totalPrice;
  List<NotifiyModel> nmList;
  ScrapBottomModalFirst(
      {super.key, required this.totalPrice, required this.nmList});

  @override
  State<ScrapBottomModalFirst> createState() => _ScrapBottomModalFirstState();
}

class _ScrapBottomModalFirstState extends State<ScrapBottomModalFirst> {
  @override
  void initState() {
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ScrapBlocY, ScrapStateY>(builder: (context, state) {
      ...
      widget.nmList.forEach((ele) {
        cm.add(state.categoryList.firstWhere(
  ...
  Widget itemWidget(
      NotifiyModel notifiyModel, ScrapStateY state, CategoryModel ele) {
    List<Consulting> consultList = [];

    consultingItemList.add(ConsultingItem(
        itemId: notifiyModel.scrapItem.id,
        price: notifiyModel.scrapItem.vendorItem.price,
        amount: notifiyModel.scrapItem.amount,
        totalPrice: notifiyModel.scrapItem.totalPrice,
        priceId: notifiyModel.scrapItem.vendorItem.itemId));
    ...
    return ListView(
      ...
      children: [
        Row(children: [
          notifiyModel.scrapItem.vendorItem.imageUrl.isEmpty
              ? Images.getIcon(
              ...
              : Image.network(
                  notifiyModel.scrapItem.vendorItem.imageUrl[0],
                  width: 56,
                  height: 56,
                  fit: BoxFit.cover,
                ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(notifiyModel.scrapItem.vendorItem.title,
                ...
                ele.vendorServiceCode == 1100
                    ? Text(
                        CustomString().pirceToString(
                            notifiyModel.scrapItem.totalPrice.toInt()),
                    ...
                    : Text(
                        CustomString().pirceToString(
                            notifiyModel.scrapItem.vendorItem.price.toInt()),
                        style: TextItems.title7.copyWith(
                            fontSize: 12,
                            fontWeight: FontWeight.w700,
                            color: ColorItems.spaceCadet)),
  ...
  _showNextPopup(List<ConsultingItem> consultingItemList) {
    ...
    widget.nmList.forEach((element) {
      consultList.add(Consulting(
          vendorProfileId: Int64(element.vendorProfileId),
          vendorServiceCode: Int64(element.vendorCode),
          consultingItemList: consultingItemList
              .where((e) => e.itemId == element.scrapItem.id)
              .toSet()));
      vendorProfileList.add(element.vendorProfileId);
    });
